...
const city=createCity('Three heritage landmarks',false);
for(const tile of city.tiles)Object.assign(tile,{terrain:'land',nature:false,elevation:0});
const sites=[['brandenburgGate',10,16],['parthenon',22,16],['tajMahal',34,16]];
for(let x=5;x<=42;x++)assert.ok(build(city,'road',[{x,y:21}]).ok);
attachCustomScenario(city,{title:'Three heritage landmarks',months:12,holdMonths:2,objectives:sites.map(([metric])=>({metric,target:1})),events:[]});
...
writeFileSync(new URL('../art/architecture/heritage-landmarks/review-challenge.city.json',import.meta.url),initial+'\n');
...
assert.ok(build(city,'brandenburgGate',[{x:10,y:16}]).ok);
