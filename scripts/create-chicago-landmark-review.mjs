...
const city=createCity('Chicago museum collection',false);
for(const tile of city.tiles)Object.assign(tile,{terrain:'land',nature:false,elevation:0});
const sites=[['adlerPlanetarium',10,16],['sheddAquarium',22,16],['artInstituteChicago',34,16]];
for(let x=5;x<=42;x++)assert.ok(build(city,'road',[{x,y:21}]).ok);
attachCustomScenario(city,{title:'Chicago museum collection',months:12,holdMonths:2,objectives:sites.map(([metric])=>({metric,target:1})),events:[]});
...
writeFileSync(new URL('../art/architecture/chicago-landmarks/review-challenge.city.json',import.meta.url),initial+'\n');
writeFileSync(new URL('../dist/assets/building-reviews/chicago-museum-collection.city.json',import.meta.url),initial+'\n');
...
assert.ok(build(city,'adlerPlanetarium',[{x:10,y:16}]).ok);
