...
const city=createCity('East Asian landmark collection',false);
for(const tile of city.tiles)Object.assign(tile,{terrain:'land',nature:false,elevation:0});
const sites=[['himejiCastle',10,16],['geunjeongjeon',22,16],['cksMemorialHall',34,16]];
for(let x=5;x<=42;x++)assert.ok(build(city,'road',[{x,y:21}]).ok);
attachCustomScenario(city,{title:'East Asian landmark collection',months:12,holdMonths:2,objectives:sites.map(([metric])=>({metric,target:1})),events:[]});
...
writeFileSync(new URL('../art/architecture/east-asian-landmarks/review-challenge.city.json',import.meta.url),initial+'\n');
writeFileSync(new URL('../dist/assets/building-reviews/east-asian-landmark-collection.city.json',import.meta.url),initial+'\n');
...
assert.ok(build(city,'bulldoze',[{x:13,y:19}]).ok);
tick(city);assert.equal(city.scenario.streak,0);assert.equal(city.scenario.status,'playing');
assert.ok(build(city,'himejiCastle',[{x:10,y:16}]).ok);
