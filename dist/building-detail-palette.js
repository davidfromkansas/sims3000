import {DECAL_NAMES,decalPolygonsOnFace,wallWorldPoint,wallCoordinates} from './building-decals.js?v=architecture-collection-48';
...
export function detailPaletteHTML(){return `<div class="building-detail-palette"><label>Detail set<select id="detailSet">${Object.entries(DETAIL_GROUPS).map(([key,g])=>`<option value="${key}">${g.name}</option>`).join('')}</select></label><input id="decalKind" type="hidden" value="1"><div class="material-swatches">${DECAL_NAMES.slice(1).map((name,i)=>`<button type="button" id="detailSwatch${i+1}" aria-pressed="${i===0}"><span id="detailTexture${i+1}" aria-hidden="true"></span><span>${name}</span></button>`).join('')}</div></div>`;}
...
export function mountBuildingDetailPalette(root,{get,kind}){
 ...
 function refresh(){const selected=Number(kind.value)||1,d=get(),key=JSON.stringify([d.facade,d.windows,d.accent]);if(selected!==previous&&!(DETAIL_GROUPS[group.value]||DETAIL_GROUPS.all).ids.includes(selected))group.value='all';previous=selected;const visible=DETAIL_GROUPS[group.value]||DETAIL_GROUPS.all;
 buttons.forEach((button,i)=>{const id=i+1;button.hidden=!visible.ids.includes(id);button.disabled=Boolean(kind.disabled);button.setAttribute('aria-pressed',String(id===selected));if(key!==artKey)$('detailTexture'+id).innerHTML=detailSwatchSVG(id,d);});artKey=key;
 }
 buttons.forEach((button,i)=>button.onclick=()=>{if(kind.disabled)return;kind.value=String(i+1);kind.onchange?.();refresh();});group.onchange=refresh;refresh();return{refresh};
}
