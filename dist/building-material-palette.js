import {BUILDING_MATERIALS,materialSurfaceColor,drawMaterialDetail} from './building-materials.js?v=architecture-collection-48';
...
export function mountBuildingMaterialPalette(root,{get,material}){
 ...
 function refresh(){const selected=Number(material.value)||0,d=get(),key=JSON.stringify([d.facade,d.windows]);if(selected!==previous&&!MATERIAL_GROUPS[group.value||'all']?.ids.includes(selected))group.value='all';previous=selected;const visible=MATERIAL_GROUPS[group.value]||MATERIAL_GROUPS.all;
  buttons.forEach((button,i)=>{button.hidden=!visible.ids.includes(i);button.disabled=Boolean(material.disabled);button.setAttribute('aria-pressed',String(i===selected));if(key!==artKey)drawMaterialSwatch($('materialTexture'+i).getContext('2d'),i,d);});artKey=key;
 }
