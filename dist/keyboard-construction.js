import {selection} from './engine.js?v=building-preview-navigation-2';
...
export function moveKeyboardCursor(r,key){const d=directions[key];if(!d)return null;anchor(r);const p=cursor(r),[x,y]=r.transform(p.x,p.y),[xx,yy]=r.inverse(x+d[0],y+d[1]),next={x:xx,y:yy};r.hover=pointInCity(r,next)?next:{...p};keepKeyboardCursorVisible(r);return r.hover;}
...
export function startKeyboardRange(r,tool){if(!keyboardRangeAllowed(tool))return null;const p={...cursor(r)};r.hover=p;r.keyboardRange={city:r.getCity(),anchor:p};r.drag=p;keepKeyboardCursorVisible(r);return p;}
