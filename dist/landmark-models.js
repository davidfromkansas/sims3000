import {US_MEMORIAL_GEOMETRY} from './us-memorial-models.js?v=architecture-collection-22';
import {eiffelTowerGeometry} from './eiffel-tower-model.js?v=architecture-collection-22';
import {greatPyramidGeometry} from './great-pyramid-model.js?v=architecture-collection-22';
import {rasterizeMiniature} from './miniature-raster.js?v=architecture-collection-22';
import {helsinkiCathedralGeometry} from './helsinki-cathedral-model.js?v=architecture-collection-22';
// Original landmark geometry shared by the city renderer and gallery previews.
export const MODELED_LANDMARKS=new Set(['bigBen','statueLiberty','chryslerBuilding','arcDeTriomphe','helsinkiCathedral','eiffelTower','greatPyramid',...Object.keys(US_MEMORIAL_GEOMETRY)]);
...
export function landmarkGeometry(type){
 ...
 if(US_MEMORIAL_GEOMETRY[type]){US_MEMORIAL_GEOMETRY[type]({box,add,ring,beam});
...
export function drawLandmarkModel(ctx,type,rotation=0){
 if(US_MEMORIAL_GEOMETRY[type]||['helsinkiCathedral','eiffelTower','greatPyramid'].includes(type)){const raster=rasterizeLandmark(type,rotation),pixels=ctx.createImageData(raster.width,raster.height);pixels.data.set(raster.data);ctx.putImageData(pixels,0,0);return;}
