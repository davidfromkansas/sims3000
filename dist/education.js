import {populationCohorts,populationCensus} from './demographics.js?v=large-city-performance-1';
...
export function advanceEducation(c){const f=c.civic,s=c.stats;if(!s.population)return;
 const target=clamp(30+(s.schoolCoverage||0)*.7+(s.activeUniversity?10:0)+(f.ordinances.reading?5:0)+(f.ordinances.juniorSports?3:0));
 ...
 f.ageEducation=f.ageEducation.map((eq,i)=>i<2?clamp(eq+(target-eq)/36):clamp(eq*(1-(1-retention)/1200)));
