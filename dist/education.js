import {populationCohorts,populationCensus} from './demographics.js?v=live-scenario-comparisons-2';
...
// Prima p.350: annual adult decay 1.91%; additive monthly library/museum/reading gains.
export const ADULT_DECAY_FACTOR=Math.pow(1-.0191,1/12);
export function adultEducationGain(c){return .09*clamp(c.stats.libraryEducationCoverage||0)/100+.07*clamp(c.stats.museumEducationCoverage||0)/100+(c.civic.ordinances.reading?.01:0);}
export function advanceEducation(c){const f=c.civic,s=c.stats;if(!s.population)return;
 const targets=educationTargets(c);
 const gain=adultEducationGain(c);
 // Teaching/retention happen before the census moves the graduating intake.
 f.ageEducation=f.ageEducation.map((eq,i)=>i<2?clamp(eq+(targets[i]-eq)/36):clamp(eq*ADULT_DECAY_FACTOR+gain));
