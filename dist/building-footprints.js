...
export function validateBuildingFootprint(v){if(!v||!Number.isInteger(v.width)||!Number.isInteger(v.height)||v.width<1||v.height<1||v.width>5||v.height>5)throw Error('Building footprints must be 1–5 tiles wide and deep.');return{width:v.width,height:v.height};}
export function parseBuildingSlot(key){const m=String(key).match(/^(0|[1-9]\d*)(?:@([1-5])x([1-5]))?$/);if(!m||m[2]==='1'&&m[3]==='1')throw Error('Invalid building style and footprint.');return{source:Number(m[1]),footprint:m[2]?{width:Number(m[2]),height:Number(m[3])}:SINGLE_TILE};}
...
export function validateBuildingFootprintVersion(replacements,designs,version){
 if(version>=146)return;
 for(const key of new Set([...Object.keys(replacements),...Object.keys(designs)])){const {footprint}=parseBuildingSlot(key);if(footprint.width>4||footprint.height>4)throw Error('Five-tile building footprints require city save version 146.');}
}
