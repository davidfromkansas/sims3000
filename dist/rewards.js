...
export function rewardRequirements(c,k){
 ...
 return k==='cityHall'?[population(20000),approval(50)]:k==='mayorHouse'?[population(5000),approval(50)]:k==='stadium'?[population(1000),approval(55),{label:'Monthly balance',current:s.balance,target:0,strict:true,money:true,met:s.balance>0}]:k==='university'?[{label:'Education quotient',current:s.education,target:70,met:s.education>=70}]:[];
}
...
export function validateRewards(v,month,version=131){if(!v||!Number.isInteger(v.lastMonth)||v.lastMonth<0||v.lastMonth>month||!v.earned||!v.streaks)throw Error('Invalid reward history.');const out=freshRewards();out.lastMonth=v.lastMonth;for(const [k,d]of Object.entries(REWARDS)){if(k==='cityHall'&&version<123){if(v.earned[k]!=null||v.streaks[k]!=null&&v.streaks[k]!==0)throw Error('City Hall requires save version 123.');continue;}const earned=v.earned[k],streak=v.streaks[k],limit=k==='university'&&version<131?6:d.months;if(earned!==null&&(!Number.isInteger(earned)||earned<0||earned>month)||!Number.isInteger(streak)||streak<0||streak>limit)throw Error('Invalid reward history.');out.earned[k]=earned;out.streaks[k]=k==='university'&&version<131?(earned===null?0:1):streak;}return out;}
