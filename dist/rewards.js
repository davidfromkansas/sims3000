...
export function validateRewards(v,month,version=123){if(!v||!Number.isInteger(v.lastMonth)||v.lastMonth<0||v.lastMonth>month||!v.earned||!v.streaks)throw Error('Invalid reward history.');const out=freshRewards();out.lastMonth=v.lastMonth;for(const [k,d]of Object.entries(REWARDS)){if(k==='cityHall'&&version<123){if(v.earned[k]!=null||v.streaks[k]!=null&&v.streaks[k]!==0)throw Error('City Hall requires save version 123.');continue;}const earned=v.earned[k],streak=v.streaks[k];if(earned!==null&&(!Number.isInteger(earned)||earned<0||earned>month)||!Number.isInteger(streak)||streak<0||streak>d.months)throw Error('Invalid reward history.');out.earned[k]=earned;out.streaks[k]=streak;}return out;}
