...
export const REWARD_APPROVAL=(100+127)*100/254;
export const STADIUM_APPROVAL=REWARD_APPROVAL;
export function rewardRequirements(c,k){
 ...
 return k==='performingArts'?[population(100000),approval(REWARD_APPROVAL)]:k==='medicalResearch'?[{label:'Year',current:c.startYear+Math.floor(c.month/12),target:1975,met:c.startYear+Math.floor(c.month/12)>=1975},population(80000),approval(STADIUM_APPROVAL),{label:'Life expectancy',current:s.lifeExpectancy,target:70,met:s.lifeExpectancy>=70}]:k==='cityHall'?[population(20000),approval(REWARD_APPROVAL)]:k==='mayorHouse'?[population(5000),approval(REWARD_APPROVAL)]:k==='countyCourthouse'?[population(25000),approval(STADIUM_APPROVAL)]:k==='stadium'?[population(150000),approval(STADIUM_APPROVAL)]:k==='university'?[{label:'Education quotient',current:s.education,target:70,met:s.education>=70}]:[];
}
