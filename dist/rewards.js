...
export const REWARDS={cityHall:{name:'City Hall',jobs:36,size:3,cost:0,upkeep:0,radius:10,amenity:25,months:1,requirement:'At least 20,000 residents and 50 approval for one simulation month.'},mayorHouse:{jobs:4,name:"Mayor's House",size:2,cost:0,upkeep:5,sprite:57,radius:7,amenity:20,months:3,requirement:'At least 5,000 residents and 50 approval for three consecutive months.'},stadium:{jobs:200,name:'Stadium',size:5,cost:75000,upkeep:25,sprite:58,radius:10,amenity:20,months:1,requirement:'Reach 150,000 residents and strong city approval (see checklist) to receive an offer at the next monthly evaluation.'},university:{jobs:500,name:'University',size:10,cost:0,upkeep:40,sprite:59,radius:9,amenity:15,months:1,requirement:'Reach 70 EQ on the city’s 0–100 education scale to receive an offer at the next monthly evaluation.'},countyCourthouse:{name:'County Courthouse',jobs:135,size:3,cost:0,upkeep:0,radius:30,amenity:0,months:1,requirement:'Reach 25,000 residents and strong city approval (see checklist) to receive an offer at the next monthly evaluation.'}};
...
export function rewardRequirements(c,k){
 ...
 return k==='cityHall'?[population(20000),approval(50)]:k==='mayorHouse'?[population(5000),approval(50)]:k==='countyCourthouse'?[population(25000),approval(STADIUM_APPROVAL)]:k==='stadium'?[population(150000),approval(STADIUM_APPROVAL)]:k==='university'?[{label:'Education quotient',current:s.education,target:70,met:s.education>=70}]:[];
}
...
export function validateRewards(v,month,version=134){if(!v||!Number.isInteger(v.lastMonth)||v.lastMonth<0||v.lastMonth>month||!v.earned||!v.streaks)throw Error('Invalid reward history.');const out=freshRewards();out.lastMonth=v.lastMonth;for(const [k,d]of Object.entries(REWARDS)){if(k==='countyCourthouse'&&version<134){if(v.earned[k]!=null||v.streaks[k]!=null&&v.streaks[k]!==0)throw Error('County Courthouse requires save version 134.');continue;}if(k==='cityHall'&&version<123){if(v.earned[k]!=null||v.streaks[k]!=null&&v.streaks[k]!==0)throw Error('City Hall requires save version 123.');continue;}const earned=v.earned[k],streak=v.streaks[k],legacy=k==='university'&&version<131||k==='stadium'&&version<133,limit=legacy?6:d.months;if(earned!==null&&(!Number.isInteger(earned)||earned<0||earned>month)||!Number.isInteger(streak)||streak<0||streak>limit)throw Error('Invalid reward history.');out.earned[k]=earned;out.streaks[k]=legacy?(earned===null?0:1):streak;}return out;}
