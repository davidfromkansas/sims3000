import {BUILDING_EDIT_PLANES,voxelPlanePoint} from './building-edit-planes.js?v=building-preview-navigation-2';
import {buildingShapeCells} from './building-shapes.js?v=building-preview-navigation-2';
import {projectBuildingPoint} from './building-footprints.js?v=building-preview-navigation-2';
...
export function buildingPlaneGuide(selection){
 ...
 if(pending)for(const i of buildingShapeCells(pending.kind,pending.start,pending.end,p.rows)){const point=voxelPlanePoint(plane,slice,i),x=(point.x-5)*.085,y=(point.y-5)*.085,bottom=point.z===0?0:.12+point.z*.14,top=.12+(point.z+1)*.14;
