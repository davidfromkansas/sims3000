import {freshEmergencyOrder,recordHazard,recordFire,validateEmergencyOrder} from './emergency-order.js?v=recreation-collection-1';
import {beginEmergencySession,pendingWarnings,refreshShelter,validateHazardWarning} from './emergency-session.js?v=recreation-collection-1';
import {freshUfo,stepUfo,validateUfo} from './ufo.js?v=recreation-collection-1';
import {freshWhirlpool,stepWhirlpool,validateWhirlpool} from './whirlpool.js?v=recreation-collection-1';
import {freshToxicCloud,stepToxicCloud,validateToxicCloud} from './toxic-cloud.js?v=recreation-collection-1';
import {freshSpaceJunk,stepSpaceJunk,validateSpaceJunk} from './space-junk.js?v=recreation-collection-1';
import {freshRiots,stepRiot,validateRiots} from './riots.js?v=recreation-collection-1';
import {freshLocusts,stepLocusts,validateLocusts} from './locusts.js?v=recreation-collection-1';
import {tunnelTiles} from './tunnels.js?v=recreation-collection-1';
import {STRUCTURES} from './structures.js?v=recreation-collection-1';
import {POWER_PLANTS} from './power.js?v=recreation-collection-1';
import {occupancy} from './utilities.js?v=recreation-collection-1';
...
export function ignite(c,x,y){const n=Math.sqrt(c.tiles.length);if(!Number.isInteger(x)||!Number.isInteger(y)||x<0||y<0||x>=n||y>=n)return{ok:false,error:'Choose a tile inside the city.'};const t=c.tiles[y*n+x];if(!combustible(t)||t.fire)return{ok:false,error:'Choose an intact building or trees to start a fire.'};beginEmergencySession(c.emergency);recordFire(c,y*n+x);t.fire=40;t.fireAge=0;return{ok:true};}
...
export function startEarthquake(c,x,y){
 ...
 beginEmergencySession(e);recordHazard(e,'earthquake');e.quakes++;e.earthquake={x,y,ring:0};return{ok:true};
}
...
export function startTornado(c,x,y){
 ...
 beginEmergencySession(e);recordHazard(e,'tornado');e.tornadoes++;
