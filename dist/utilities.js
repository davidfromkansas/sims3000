import {waterBaseNeed} from './utility-demand.js?v=water-grid-report-1';
import {pipeCoverage} from './water-coverage.js?v=water-grid-report-1';
import {conservationDemand} from './conservation.js?v=water-grid-report-1';
import {industrialJobs} from './industry.js?v=water-grid-report-1';
import {tradeCapacity} from './region.js?v=water-grid-report-1';
...
export function processGarbage(c){
 ...
 for(const d of dumps){if(!t.roadIds?.some(id=>d.roadIds?.includes(id)))continue;const amount=Math.min(t.waste,LANDFILL_CAPACITY-d.garbage);if(amount<=0)continue;d.garbage+=amount;t.waste-=amount;collected+=amount;if(t.waste<1e-8){t.waste=0;break;}}}return collected;
}
