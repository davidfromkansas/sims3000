import {MAX_SCENARIO_VARIABLES} from './scenario-variables.js?v=scenario-variable-manager-1';
import {BUILDING_UTILITY_METRICS} from './scenario-building-utilities.js?v=scenario-variable-manager-1';
import {NEIGHBOR_METRICS} from './scenario-neighbor-metrics.js?v=scenario-variable-manager-1';
import {isBuildingLotRoot,buildingLotMembers} from './building-lots.js?v=scenario-variable-manager-1';
import {averageWaterPollution,averageRoadTraffic,surplusPower,surplusWater} from './city-measures.js?v=scenario-variable-manager-1';
import {outstandingLoanPayments} from './loan-debt.js?v=scenario-variable-manager-1';
import {CALENDAR_MONTHS,calendarMonthIndex} from './scenario-calendar.js?v=scenario-variable-manager-1';
import {tileIndex} from './city-grid.js?v=scenario-variable-manager-1';
import {STRUCTURE_METRICS} from './scenario-structures.js?v=scenario-variable-manager-1';
import {inScenarioArea} from './scenario-area.js?v=scenario-variable-manager-1';
import {ORDINANCES} from './ordinances.js?v=scenario-variable-manager-1';
import {LANDMARKS,landmarkRoots} from './landmarks.js?v=scenario-variable-manager-1';
import {businessRoots} from './business.js?v=scenario-variable-manager-1';
...
// Independent education measures are available to goals, conditions and programs.
Object.assign(CUSTOM_METRICS,{
 adultEducation:{name:'Adult education',direction:'at least',max:100,initial:41,read:c=>c.civic.adultEducation},
 childEducationCoverage:{name:'School access · ages 0–14 (%)',direction:'at least',max:100,initial:90,read:c=>c.stats.childEducationCoverage},
 collegeEducationCoverage:{name:'College access · ages 15–24 (%)',direction:'at least',max:100,initial:90,read:c=>c.stats.collegeEducationCoverage},
 libraryEducationCoverage:{name:'Library access · adults (%)',direction:'at least',max:100,initial:90,read:c=>c.stats.libraryEducationCoverage},
 museumEducationCoverage:{name:'Museum access · adults (%)',direction:'at least',max:100,initial:90,read:c=>c.stats.museumEducationCoverage}
});
...
for(let i=0;i<MAX_SCENARIO_VARIABLES;i++)CUSTOM_METRICS[`variable${i+1}`]={name:`Scenario variable ${i+1}`,direction:'at least',min:-1000000,max:1000000,integer:true,initial:1,read:c=>c.scenario?.id==='custom'?(c.scenario.variables?.[i]??0):0};
