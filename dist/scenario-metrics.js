import {STRUCTURE_METRICS} from './scenario-structures.js?v=scenario-calculations-1';
import {inScenarioArea} from './scenario-area.js?v=scenario-calculations-1';
import {ORDINANCES} from './ordinances.js?v=scenario-calculations-1';
import {LANDMARKS,landmarkRoots} from './landmarks.js?v=scenario-calculations-1';
import {businessRoots} from './business.js?v=scenario-calculations-1';
...
Object.assign(CUSTOM_METRICS,{
 farms:{name:'Developed farms',direction:'at least',max:256,initial:3,spatial:true,read:(c,area)=>c.tiles.filter(t=>inScenarioArea(t,area)&&t.type==='industrial'&&t.industry==='farm'&&t.farmRoot===t.y*48+t.x&&t.level>0&&!t.rubble&&!t.radiation).length},
...
Object.assign(CUSTOM_METRICS,Object.fromEntries([['residential','Developed residential buildings'],['commercial','Developed commercial buildings'],['industrial','Developed industrial buildings']].map(([type,name])=>['buildings'+type[0].toUpperCase()+type.slice(1),{name,direction:'at least',max:2304,initial:10,spatial:true,integer:true,read:(c,area)=>c.tiles.filter(t=>inScenarioArea(t,area)&&t.type===type&&t.level>0&&!t.rubble&&!t.radiation&&(t.industry!=='farm'||t.farmRoot===t.y*48+t.x)).length}])));

Object.assign(CUSTOM_METRICS,STRUCTURE_METRICS);

CUSTOM_METRICS.abandonedBuildings={name:'Abandoned RCI buildings',direction:'at most',integer:true,max:2304,initial:0,spatial:true,read:(c,area)=>c.tiles.filter(t=>['residential','commercial','industrial'].includes(t.type)&&!t.level&&(t.abandonedLevel||t.historicalLevel)&&!t.rubble&&inScenarioArea(t,area)&&(t.industry!=='farm'||t.farmRoot===t.y*48+t.x)).length};
