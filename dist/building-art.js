import {buildingSlot,parseBuildingSlot,tileBuildingFootprint} from './building-footprints.js?v=live-scenario-comparisons-2';
...
export const REPLACEABLE_STYLES={
 ...
 3:{name:'Neighborhood shops',group:'commercial',level:1},77:{name:'Corner shop and apartment',group:'commercial',level:1},80:{name:'Limestone office block',group:'commercial',level:2,introducedVersion:145},4:{name:'Commercial mid-rise',group:'commercial',level:2},5:{name:'Classic office tower',group:'commercial',level:3},75:{name:'Stepped glass office',group:'commercial',level:3},
 6:{name:'Small factory',group:'industrial',level:1},79:{name:'Sawtooth factory',group:'industrial',level:1},7:{name:'Industrial works',group:'industrial',level:2},8:{name:'Heavy industrial complex',group:'industrial',level:3},54:{name:'Small clean-industry workshop',group:'industrial',level:1},55:{name:'Clean-industry campus',group:'industrial',level:2},56:{name:'High-tech industrial complex',group:'industrial',level:3}
};
...
export function cityZonedSprite(t,city,view){const sprite=zonedSprite(t,city.seed,city.buildingReplacements,city),base=baseZonedSprite(t,city.seed),key=buildingStyleKey(city,t),variant=base===0&&view?.variedHomes?76:base===3&&view?.variedShops?77:base===1&&view?.variedApartments?78:base===6&&view?.variedFactories?79:null;if(variant===null||(t.lotRoot!=null&&variant!==78&&variant!==79)||sprite!==base||Object.hasOwn(city.buildingReplacements||{},key)||city.buildingDesigns?.[key])return sprite;const root=t.lotRoot==null?t:city.tiles[t.lotRoot];return ((Math.imul(root.x+1,73856093)^Math.imul(root.y+1,19349663)^city.seed)>>>3)&1?variant:base;}

export function validateBuildingArtVersion(replacements,designs,version){
 const sources=[...Object.keys(replacements).map(key=>parseBuildingSlot(key).source),...Object.values(replacements),...Object.keys(designs).map(key=>parseBuildingSlot(key).source)];
 for(const source of sources){const style=REPLACEABLE_STYLES[source];if(style?.introducedVersion>version)throw Error(`${style.name} requires city save version ${style.introducedVersion}.`);}
}
