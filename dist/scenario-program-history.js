import {PROGRAM_LIMITS,startScenarioProgram,nextScenarioProgramAction,validateScenarioProgramCursor} from './scenario-programs.js?v=scenario-neighbor-status-1';
import {executeScenarioAction,eventConditionMet,validateEventProgress} from './scenario-events.js?v=scenario-neighbor-status-1';
...
export function validateProgramInvocation(value,programs,{startMonth,month}){
 ...
 for(const [index,receipt]of value.receipts.entries()){
  ...
  lastMonth=receipt.month;const textAction=['announcement','popup','ending'].includes(action.type),message=value.text.find(t=>t?.receipt===index)?.message;
 ...
 const textIndices=actions.map((a,i)=>['announcement','popup','ending'].includes(a.type)?i:null).filter(i=>i!==null).slice(-PROGRAM_TEXT_HISTORY);
 if(value.text.length!==textIndices.length)throw Error('Missing routine message history.');
 const text=value.text.map((t,i)=>{if(!t||t.receipt!==textIndices[i]||typeof t.message!=='string'||!t.message||t.message.length>4000)throw Error('Invalid routine message history.');return{receipt:t.receipt,message:t.message};});
