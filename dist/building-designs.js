import {REPLACEABLE_STYLES,baseZonedSprite} from './building-art.js?v=building-designer-1';
...
export function exportBuildingDesign(design){return JSON.stringify({format:'SIMS3000-building',version:1,design:validateBuildingDesign(design)},null,2);}
export function importBuildingDesign(text){if(typeof text!=='string'||text.length>8192)throw Error('Building files must be smaller than 8 KB.');let file;try{file=JSON.parse(text);}catch{throw Error('This is not a valid building file.');}if(file?.format!=='SIMS3000-building'||file.version!==1)throw Error('Choose a SIMS3000 building file.');return validateBuildingDesign(file.design);}
