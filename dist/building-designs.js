import {validateBuildingBlocks,drawBuildingBlocks} from './building-blocks.js?v=building-blocks-1';
import {REPLACEABLE_STYLES,baseZonedSprite} from './building-art.js?v=building-blocks-1';
...
export function exportBuildingDesign(design){return JSON.stringify({format:'SIMS3000-building',version:design.blocks?2:1,design:validateBuildingDesign(design)},null,2);}
export function importBuildingDesign(text){if(typeof text!=='string'||text.length>8192)throw Error('Building files must be smaller than 8 KB.');let file;try{file=JSON.parse(text);}catch{throw Error('This is not a valid building file.');}if(file?.format!=='SIMS3000-building'||![1,2].includes(file.version)||(file.version===2)!==(file.design?.blocks!==undefined))throw Error('Choose a SIMS3000 building file.');return validateBuildingDesign(file.design);}
