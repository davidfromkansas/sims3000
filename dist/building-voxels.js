import {wallDecorationFrame,clipWallDecoration} from './building-face-clipping.js?v=architecture-collection-39';
import {blockCornerHeights} from './building-block-geometry.js?v=architecture-collection-39';
import {buildingSurfaceDetail,drawBuildingSurfaceDetail} from './building-surface-details.js?v=architecture-collection-39';
import {floorSurfaceMaterial,splitBuildingFloors} from './building-floor-paint.js?v=architecture-collection-39';
import {projectBuildingPoint} from './building-footprints.js?v=architecture-collection-39';
import {buildingShapeCells} from './building-shapes.js?v=architecture-collection-39';
import {materialSurfaceColor,drawMaterialDetail} from './building-materials.js?v=architecture-collection-39';
...
export function buildingVoxelFaces(voxels,rotation=0,footprint={width:1,height:1},geometry){
 ...
 for(const i of order){const x=i%10,y=Math.floor(i/10),x0=(x-5)*.085,y0=(y-5)*.085,corners=[[x0,y0],[x0+.085,y0],[x0+.085,y0+.085],[x0,y0+.085]];
  for(let level=0;level<24;level++){if(!voxelOccupied(voxels,i,level))continue;const bottom=level===0?0:.12+level*.14,top=.12+(level+1)*.14,shape=Number(geometry?.[level*100+i]||0),heights=blockCornerHeights(shape),tops=heights.map(h=>bottom+(top-bottom)*h);
   for(const side of visible){const [dx,dy]=directions[side],nx=x+dx,ny=y+dy;if(nx>=0&&nx<10&&ny>=0&&ny<10&&voxelOccupied(voxels,ny*10+nx,level)&&!Number(geometry?.[level*100+ny*10+nx]||0))continue;if(!heights[side]&&!heights[(side+1)%4])continue;const a=corners[side],b=corners[(side+1)%4];faces.push({x,y,side,...(shape?{shape}:{}),from:level,to:level+1,points:[[...a,bottom],[...b,bottom],[...b,tops[(side+1)%4]],[...a,tops[side]]]});}
   if(shape||!voxelOccupied(voxels,i,level+1)){const points=corners.map((p,k)=>[...p,tops[k]]),projected=points.map(p=>projectBuildingPoint(p,rotation,footprint)),area=projected.reduce((sum,p,k)=>{const q=projected[(k+1)%4];return sum+p[0]*q[1]-q[0]*p[1];},0);if(!shape||area>1e-8)faces.push({x,y,side:4,...(shape?{shape}:{}),from:level+1,to:level+1,points});}
  }
 }return faces;
}
