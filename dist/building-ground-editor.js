import {projectedTowerOcclusion} from './building-tower-geometry.js?v=architecture-collection-55';
import {groundFaces,groundMaterial,paintGround,connectedGround} from './building-ground-paint.js?v=architecture-collection-55';
import {projectedBuildingSurfaces,pickBuildingSurface} from './building-surface-picking.js?v=architecture-collection-55';
import {buildingPropPickFaces,pickBuildingProp} from './building-prop-picking.js?v=architecture-collection-55';
import {BUILDING_MATERIALS,buildingMaterialName} from './building-materials.js?v=architecture-collection-55';
export function mountBuildingGroundEditor(canvas,{get,camera,mode,material,status,apply,render}){
 ...
 function sample(face){if(!face)return;const value=groundMaterial(get(),face.index);if(value>=0)material.value=String(value);status.textContent=value>=0?`Selected ${buildingMaterialName(value,get())}. Ground is unchanged.`:'Original ground. Your selected material is unchanged.';}
