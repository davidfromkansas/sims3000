import {powerBaseNeed,waterBaseNeed} from './utility-demand.js?v=city-hall-neighborhood-1';
import {EDUCATION_LAYERS,EDUCATION_LAYER_LEGEND,educationLayerProfile,educationLayerColor} from './education-layers.js?v=city-hall-neighborhood-1';
import {drawNavigationPrecincts,PRECINCT_LAYERS,PRECINCT_LEGEND} from './service-areas.js?v=city-hall-neighborhood-1';
import {mapSize} from './city-grid.js?v=city-hall-neighborhood-1';
...
export function installNavigation(renderer){
 ...
 const refreshLegend=()=>{legend.textContent=navigationLegend(select.value)+(PRECINCT_LAYERS[select.value]?' · '+PRECINCT_LEGEND:'')+' · Click to jump · Outline: ground view';};select.onchange=()=>{refreshLegend();draw();};renderer.setNavigationLayer=layer=>{if(!Object.hasOwn(NAV_LAYERS,layer))throw Error('Unknown navigation map layer.');select.value=layer;panel.open=true;refreshLegend();draw();};refreshLegend();panel.addEventListener('toggle',draw);map.addEventListener('focus',draw);map.addEventListener('blur',draw);renderer.drawNavigation=draw;draw();
}
