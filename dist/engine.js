import {freshDemographics,advanceDemographics,validateDemographics} from './demographics.js?v=scenario-stages-1';
import {powerMonth} from './power-accounting.js?v=scenario-stages-1';
import {initialAgeEducation,advanceEducationCohorts} from './education.js?v=scenario-stages-1';
import {validateBuildingReplacements} from './building-art.js?v=scenario-stages-1';
import {conservationDemand} from './conservation.js?v=scenario-stages-1';
import {LANDMARKS,validateLandmarks} from './landmarks.js?v=scenario-stages-1';
import {maybeUfo} from './ufo.js?v=scenario-stages-1';
import {maybeWhirlpool} from './whirlpool.js?v=scenario-stages-1';
import {maybeToxicCloud} from './toxic-cloud.js?v=scenario-stages-1';
import {maybeSpaceJunk} from './space-junk.js?v=scenario-stages-1';
import {runScenarioEventBatch,scenarioEventPresentation} from './scenario-events.js?v=scenario-stages-1';
import {advanceUnrest} from './riots.js?v=scenario-stages-1';
import {maybeLocusts} from './locusts.js?v=scenario-stages-1';
import {wasteState,wasteMonth} from './waste-accounting.js?v=scenario-stages-1';
import {BUSINESSES,freshBusiness,advanceBusiness,businessStats,validateBusiness,businessRoots,businessOffer} from './business.js?v=scenario-stages-1';
import {TECHNOLOGY,available} from './technology.js?v=scenario-stages-1';
import {proposeTunnel,installTunnel,removeTunnels,tunnelStats,validateTunnels,tunnelAt} from './tunnels.js?v=scenario-stages-1';
import {REWARDS,freshRewards,advanceRewards,rewardStats,validateRewards} from './rewards.js?v=scenario-stages-1';
import {normalizeIndustry,advanceIndustry,industryStats,industrialJobs} from './industry.js?v=scenario-stages-1';
import {STRUCTURES} from './structures.js?v=scenario-stages-1';
import {RECREATION,recreationStats} from './recreation.js?v=scenario-stages-1';
import {reportSnapshot,HISTORY_LIMIT,cleanHistory} from './reports.js?v=scenario-stages-1';
import {LANDSCAPE,planLandscape} from './landscape.js?v=scenario-stages-1';
import {POWER_PLANTS,plantCapacity,advancePower,powerStats} from './power.js?v=scenario-stages-1';
import {advanceScenario,validateScenario} from './scenarios.js?v=scenario-stages-1';
import {explodePlant,ignite,freshEmergency,emergencyStats,maybeIgnite,maybeEarthquake,maybeTornado,validateEmergency} from './emergency.js?v=scenario-stages-1';
import {freshRegion,resetTrades,tradeCapacity,importGarbage,exportGarbage,regionBudget,settleRegion,validateRegion} from './region.js?v=scenario-stages-1';
import {FACILITIES,recomputeFacilities,advanceFacilities} from './facilities.js?v=scenario-stages-1';
import {streetGraph,rampCrossings} from './highway.js?v=scenario-stages-1';
import {STATIONS} from './rail.js?v=scenario-stages-1';
import {freshTransport,recomputeTransport,advanceTransit,validateTransport,roadNeighbors,bridgePlan} from './transport.js?v=scenario-stages-1';
import {SERVICES,freshCivic,recomputeCivic,advanceCivic,validateCivic} from './civic.js?v=scenario-stages-1';
import {queueBudgetReview,freshFinance,changeBudget,takeLoan,settleLoans,advanceRoads,recomputeEnvironment,landDensityLimit,budgetForecast,validateFinance} from './economy.js?v=scenario-stages-1';
import {WASTE_STRUCTURES,wastePower,advanceWaste,WATER_STRUCTURES,advanceWater,recomputeWater,garbageStats,processGarbage,occupancy,LANDFILL_CAPACITY} from './utilities.js?v=scenario-stages-1';
// SIMS3000 milestone 3. Gameplay constants are explicit approximations; see docs/manual-fidelity.md.
export const SIZE=48, VERSION=77;
...
export function recompute(c){
 ...
 c.stats.demand.commercial=clamp(c.stats.demand.commercial+Math.min(40,facilities.activeAirports*20+facilities.activeSeaports*8),-100,100);c.stats.demand.industrial=clamp(c.stats.demand.industrial+Math.min(40,facilities.activeAirports*5+facilities.activeSeaports*20),-100,100);
 ...
 Object.assign(c.stats,regionBudget(c));for(const sector of ['commercial','industrial'])c.stats.demand[sector]=clamp(c.stats.demand[sector]+Math.min(15,c.stats.tradeConnections*3),-100,100);Object.assign(c.stats,budgetForecast(c));
 return c.stats;
}
...
export function validateSave(data){
 ...
 const c=createCity(data.name,false);c.buildingReplacements=data.version>=58?validateBuildingReplacements(data.buildingReplacements):{};if(data.version>=16){if(!Number.isInteger(data.startYear)||data.startYear<1900||data.startYear>2050||typeof data.mayorName!=='string'||data.mayorName.length>50||!['easy','medium','hard'].includes(data.difficulty))throw Error('Invalid city setup.');c.startYear=data.startYear;c.mayorName=data.mayorName;c.difficulty=data.difficulty;}if(data.seed!==undefined){if(!Number.isInteger(data.seed)||data.seed<0||data.seed>4294967295)throw Error('Invalid city seed.');c.seed=data.seed;}c.region=data.version>=9?validateRegion(data.region,data.month,SIZE*SIZE):freshRegion();c.transport=data.version>=5?validateTransport(data.transport):freshTransport();c.civic=data.version>=4?validateCivic({...data.civic,...(data.version<62?{ageEducation:initialAgeEducation(data.version<30?data.civic?.education:data.civic?.youthEducation,data.version<30?data.civic?.education:data.civic?.adultEducation)}:{}),...(data.version<30?{youthEducation:data.civic?.education,adultEducation:data.civic?.education}:{}),ordinances:{...data.civic?.ordinances,...(data.version<57?{powerConservation:false,waterConservation:false}:{}),...(data.version<56?{freeClinics:false,juniorSports:false}:{}),...(data.version<28?{parkingFines:false}:{}),...(data.version<27?{carpool:false,cleanAir:false}:{}),...(data.version<13?{trashPresort:false}:{}),...(data.version<19?{cleanWater:false}:{})}}):freshCivic();c.business=data.version>=29?validateBusiness(data.business,data.month,data.version):freshBusiness();c.rewards=data.version>=20?validateRewards(data.rewards,data.month):freshRewards();c.funds=data.funds;c.month=data.month;c.starter=!!data.starter;c.finance=data.version>=3?validateFinance(data.finance,data.month,data.version):freshFinance();
 ...
 c.scenario=data.version>=11?validateScenario(data.scenario,c.month):null;
