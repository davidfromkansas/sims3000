import {endingRankChoices,remapEndingRank} from './scenario-ending-ranks.js?v=east-asian-landmarks-1';
import {removeScenarioVariable,scenarioVariableIndex} from './scenario-variable-references.js?v=east-asian-landmarks-1';
import {PROGRAM_LIMITS} from './scenario-programs.js?v=east-asian-landmarks-1';
import {defaultTornadoSettings,TORNADO_DIRECTIONS,TORNADO_SPEEDS} from './tornado-settings.js?v=east-asian-landmarks-1';
import {SCENARIO_EVENTS,CONDITION_METRICS} from './scenario-events.js?v=east-asian-landmarks-1';
import {CUSTOM_METRICS} from './scenario-metrics.js?v=east-asian-landmarks-1';
import {CALCULATIONS} from './scenario-variables.js?v=east-asian-landmarks-1';
import {SCENARIO_SOUNDS} from './scenario-sounds.js?v=east-asian-landmarks-1';
import {BUSINESSES} from './business.js?v=east-asian-landmarks-1';
import {REWARDS} from './rewards.js?v=east-asian-landmarks-1';
import {dateInputValue,readMetricTarget} from './scenario-calendar.js?v=east-asian-landmarks-1';
import {validateScenarioProgramDefinitions} from './scenario-program-definitions.js?v=east-asian-landmarks-1';
...
export function mountScenarioProgramEditor(root,{size=48,initial=[],available=[],availableRanks=[],ranks=()=>[],entryTargets=()=>[],variables=()=>Array.from({length:4},(_,i)=>({name:'Variable '+(i+1),initial:0})),onChange=()=>{}}={}){
 ...
 function actionFields(parent,a){
  ...
  if(a.type==='tornado'){
   const t=a.tornado??(a.tornado=defaultTornadoSettings());
   field(parent,'Travel direction',t.direction,v=>t.direction=v,{choices:optionsOf(TORNADO_DIRECTIONS)});
   field(parent,'Tornado intensity (1–100)',t.intensity,v=>t.intensity=v,{type:'number',min:1,max:100});
   field(parent,'Travel distance (tiles)',t.distance,v=>t.distance=v,{type:'number',min:1,max:256});
   field(parent,'Travel speed',t.speed,v=>t.speed=v,{choices:optionsOf(TORNADO_SPEEDS)});
   check(parent,'Early tornado warning',t.warning,v=>t.warning=v);
   parent.append(el('p','The starting tile counts toward distance; the storm ends at the map edge. Intensity changes surface damage chance. Warning gives eight response steps to sound the siren.'));
  }
  ...
  if(a.type==='variable'){
   field(parent,'Variable slot',a.variable,v=>a.variable=Number(v),{choices:variables().map((v,i)=>[i,v.name||'Unnamed variable'])});
