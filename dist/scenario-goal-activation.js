...
export function validateGoalActivations(v,definition,startMonth,lastChecked,events,progress){
 ...
 const activated=v.map((month,i)=>{
  ...
  const sources=definition.events.map((e,j)=>({e,state:events?.[j]})).filter(({e,state})=>e.type==='addGoal'&&e.goal===i&&state?.status==='triggered'&&state.runs>0);
  if(startup?month!==startMonth:month===null?sources.length>0:!Number.isInteger(month)||month<=startMonth||month>lastChecked||!sources.some(({e,state})=>month>=startMonth+e.month&&month<=state.month))throw Error('Invalid scenario goal activation history.');
  return month;
 });
 if(progress){
