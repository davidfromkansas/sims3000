import {presenterMarkup} from './scenario-presenters.js?v=keyboard-construction-1';
import {escapeAnnouncement} from './scenario-announcements.js?v=keyboard-construction-1';
...
export function showPendingScenarioPopup(ui){const c=ui.city(),pending=pendingScenarioPopup(c);if(!pending)return false;const {definition,state}=pending;
 ui.dialog('Scenario message',`${presenterMarkup(definition.presenter)}<p style="white-space:pre-wrap">${escapeAnnouncement(state.message)}</p><div class="actions"><button id="dismissScenarioPopup" class="primary">Return to city</button>${definition.showStatus?'<button id="popupScenarioStatus">Scenario status</button>':''}</div>`);
