// One character per column, floor and side: 0 inherits column paint; 1–5
...
export function validateFloorPaint(value){if(typeof value!=='string'||value.length!==FLOOR_PAINT_SIZE||/[^0-5]/.test(value))throw Error('Floor paint needs 12,000 valid surface entries.');return value;}
...
export function floorSurfaceMaterial(design,face){const value=Number(design.surfacePaint?.[floorPaintKey(face)]||0);return value?value-1:design.materials?.[(face.y*10+face.x)*5+face.side]||0;}
export function paintFloorSurfaces(design,faces,material){if(!Number.isInteger(material)||material<0||material>4)throw Error('Choose a valid building material.');const next=(design.surfacePaint?validateFloorPaint(design.surfacePaint):'0'.repeat(FLOOR_PAINT_SIZE)).split('');for(const face of faces)next[floorPaintKey(face)]=String(material+1);return next.join('');}
