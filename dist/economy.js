import {outstandingLoanPayments} from './loan-debt.js?v=country-club-1';
import {REWARDS,rewardSize,rewardRoots,rewardActive,CITY_HALL_EFFECTS} from './rewards.js?v=country-club-1';
import {industrialJobs,industryPollution} from './industry.js?v=country-club-1';
import {RECREATION,recreationRoots,recreationActive} from './recreation.js?v=country-club-1';
import {POWER_PLANTS} from './power.js?v=country-club-1';
import {civicSpending,ordinanceRevenue} from './civic.js?v=country-club-1';
import {occupancy} from './utilities.js?v=country-club-1';
...
export function recomputeEnvironment(c){
 const cleanAir=c.civic.ordinances.cleanAir?.8:1;const tiles=c.tiles,n=Math.sqrt(tiles.length),air=new Float64Array(tiles.length),pollution=new Float64Array(tiles.length),green=new Float64Array(tiles.length),amenity=new Float64Array(tiles.length),health=new Float64Array(tiles.length),shore=new Uint8Array(tiles.length),club=new Float64Array(tiles.length),courtValue=new Float64Array(tiles.length),research=new Float64Array(tiles.length),campus=new Float64Array(tiles.length),campusAura=new Float64Array(tiles.length),stadium=new Float64Array(tiles.length),stadiumAura=new Float64Array(tiles.length),civicValue=new Float64Array(tiles.length),civicAura=new Float64Array(tiles.length),arts=new Float64Array(tiles.length),artsAura=new Float64Array(tiles.length);
 ...
 for(const t of rewardRoots(c)){if(!rewardActive(c,t))continue;const d=REWARDS[t.type];const center={x:t.x+Math.floor(rewardSize(t)/2),y:t.y+Math.floor(rewardSize(t)/2)};spread(center,d.radius,(i,f)=>{amenity[i]+=d.amenity*f;});if(t.type==='countryClub'){spread(center,15,(i,f)=>{club[i]+=f;air[i]-=25*f;pollution[i]+=25*f;});}if(t.type==='countyCourthouse'){spread(center,30,(i,f)=>{const sector=tiles[i].type;courtValue[i]+=(sector==='residential'?20:sector==='commercial'?25:0)*f;});}if(t.type==='historicStatue'||t.type==='lighthouse'){const statue=t.type==='historicStatue';spread(center,d.radius,(i,f)=>{const sector=tiles[i].type;civicValue[i]+=(sector==='residential'?(statue?18:12):sector==='commercial'?(statue?18:25):sector==='industrial'?(statue?5:0):0)*f;civicAura[i]+=(statue?1:2)*100/254*f;});if(!statue){spread(center,10,(i,f)=>{air[i]+=2.4*f;});spread(center,5,(i,f)=>{pollution[i]+=2.4*f;});}}if(t.type==='performingArts'){spread(center,10,(i,f)=>{air[i]+=2*f;arts[i]+=f;});spread(center,5,(i,f)=>{pollution[i]+=2*f;});spread(center,20,(i,f)=>{artsAura[i]+=f;});}if(t.type==='mayorHouse'||t.type==='cityHall'){const hall=t.type==='cityHall';spread(center,10,(i,f)=>{const sector=tiles[i].type;civicValue[i]+=(sector==='residential'?(hall?10:23):sector==='commercial'?(hall?25:20):sector==='industrial'?(hall?0:2):0)*f;});spread(center,hall?20:10,(i,f)=>{civicAura[i]+=(hall?3:2)*100/254*f;});}if(t.type==='stadium'){spread(center,15,(i,f)=>{air[i]+=15*f;pollution[i]+=20*f;});spread(center,16,(i,f)=>{stadium[i]+=f;});spread(center,20,(i,f)=>{stadiumAura[i]+=f;});}if(t.type==='university'){spread(center,10,(i,f)=>{air[i]+=60*f;campus[i]+=f;});spread(center,5,(i,f)=>{pollution[i]+=60*f;});spread(center,20,(i,f)=>{campusAura[i]+=f;});}if(t.type==='medicalResearch'){spread(center,10,(i,f)=>{air[i]+=5.4*f;});spread(center,5,(i,f)=>{pollution[i]+=7.2*f;});spread(center,15,(i,f)=>{research[i]+=f;});}if(t.type==='cityHall'){spread(center,CITY_HALL_EFFECTS.airRadius,(i,f)=>{air[i]+=CITY_HALL_EFFECTS.air*f;});spread(center,CITY_HALL_EFFECTS.waterRadius,(i,f)=>{pollution[i]+=CITY_HALL_EFFECTS.water*f;});}}
 let totalValue=0,totalAir=0,count=0;
 for(let i=0;i<tiles.length;i++){const t=tiles[i];t.recreationHealth=Math.min(5,health[i]);t.airPollution=clamp(air[i]-Math.min(20,green[i]),0,100);t.waterPollution=clamp(pollution[i]-Math.min(8,green[i]*.25),0,100);t.landValue=t.radiation?1:clamp(48+Math.min(30,amenity[i])+(shore[i]?8:0)+(t.access?10:0)-t.airPollution*.45-t.waterPollution*.15-t.waste*.3-(100-c.finance.roadCondition)*.15,1,100);t.performingArtsAura=artsAura[i]*300/254;t.performingArtsLandValue=t.radiation?0:arts[i]*(t.type==='residential'?12:t.type==='commercial'?20:0);t.countryClubLandValue=t.radiation?0:club[i]*(t.type==='residential'?20:t.type==='commercial'?25:0);t.countryClubAura=club[i]*100/254;t.courthouseLandValue=t.radiation?0:courtValue[i];t.civicRewardAura=civicAura[i];t.civicRewardLandValue=t.radiation?0:civicValue[i];t.stadiumAura=stadiumAura[i]*200/254;t.stadiumCrimePressure=stadium[i]*20;t.stadiumLandValue=t.radiation?0:stadium[i]*(t.type==='residential'?-20:t.type==='commercial'?14:0)||0;t.universityAura=campusAura[i]*100/254;t.universityCrimePressure=campus[i]*15;t.universityLandValue=t.radiation?0:campus[i]*(t.type==='residential'?5:t.type==='commercial'?15:0);t.medicalResearchAura=research[i]*200/254;t.medicalResearchLandValue=!t.radiation&&['residential','commercial'].includes(t.type)?research[i]*10:0;t.landValue=clamp(t.landValue+t.countryClubLandValue+t.courthouseLandValue+t.medicalResearchLandValue+t.universityLandValue+t.stadiumLandValue+t.civicRewardLandValue+t.performingArtsLandValue,1,100);t.environmentLandValue=t.landValue;if(SECTORS.includes(t.type)){totalValue+=t.landValue;totalAir+=t.airPollution;count++;}}
