...
import {REWARDS,rewardRoots,rewardActive,CITY_HALL_EFFECTS} from './rewards.js?v=education-capacity-1';
import {industrialJobs,industryPollution} from './industry.js?v=education-capacity-1';
import {RECREATION,recreationRoots,recreationActive} from './recreation.js?v=education-capacity-1';
import {POWER_PLANTS} from './power.js?v=education-capacity-1';
import {civicSpending,ordinanceRevenue} from './civic.js?v=education-capacity-1';
...
export function recomputeEnvironment(c){
 ...
 for(const t of rewardRoots(c)){if(!rewardActive(c,t))continue;const d=REWARDS[t.type];const center={x:t.x+Math.floor(d.size/2),y:t.y+Math.floor(d.size/2)};spread(center,d.radius,(i,f)=>{amenity[i]+=d.amenity*f;});if(t.type==='cityHall'){spread(center,CITY_HALL_EFFECTS.airRadius,(i,f)=>{air[i]+=CITY_HALL_EFFECTS.air*f;});spread(center,CITY_HALL_EFFECTS.waterRadius,(i,f)=>{pollution[i]+=CITY_HALL_EFFECTS.water*f;});}}
