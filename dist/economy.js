import {outstandingLoanPayments} from './loan-debt.js?v=civic-landmarks-1';
import {REWARDS,rewardSize,rewardRoots,rewardActive,CITY_HALL_EFFECTS} from './rewards.js?v=civic-landmarks-1';
import {industrialJobs,industryPollution} from './industry.js?v=civic-landmarks-1';
import {RECREATION,recreationRoots,recreationActive} from './recreation.js?v=civic-landmarks-1';
import {POWER_PLANTS} from './power.js?v=civic-landmarks-1';
import {civicSpending,ordinanceRevenue} from './civic.js?v=civic-landmarks-1';
...
export function recomputeEnvironment(c){
 const cleanAir=c.civic.ordinances.cleanAir?.8:1;const tiles=c.tiles,n=Math.sqrt(tiles.length),air=new Float64Array(tiles.length),pollution=new Float64Array(tiles.length),green=new Float64Array(tiles.length),amenity=new Float64Array(tiles.length),health=new Float64Array(tiles.length),shore=new Uint8Array(tiles.length),research=new Float64Array(tiles.length),campus=new Float64Array(tiles.length),campusAura=new Float64Array(tiles.length),stadium=new Float64Array(tiles.length),stadiumAura=new Float64Array(tiles.length),civicValue=new Float64Array(tiles.length),civicAura=new Float64Array(tiles.length),arts=new Float64Array(tiles.length),artsAura=new Float64Array(tiles.length);
