import {landDensityLimit} from './economy.js?v=recent-construction-1';
import {needsEstablishedWater} from './water-service.js?v=recent-construction-1';
import {buildingLotMembers,isBuildingLotRoot} from './building-lots.js?v=recent-construction-1';
...
export function zoneGrowthReport(c){const rows=[];for(const t of c.tiles){if(!['residential','commercial','industrial'].includes(t.type)||t.industry==='farm'||!isBuildingLotRoot(c,t))continue;const members=buildingLotMembers(c,t),conditions=zoneGrowthConditions(c,t,members),reasons=[...conditions.blocked];
 ...
 rows.push({x:t.x,y:t.y,type:t.type,tiles:members.length,level:t.level,density:t.density,limit:conditions.limit,demand:conditions.demand,reasons,state:!conditions.usable?'blocked':t.level>=conditions.limit?'capped':conditions.demand<=0?'demand':!conditions.ageReady?'maturing':'ready'});}
 return rows;}
