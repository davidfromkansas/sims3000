...
const saved=serializeCity(c),loaded=validateSave(JSON.parse(saved));assert.equal(loaded.version,142);assert.deepEqual(loaded.emergency.policeUnits,c.emergency.policeUnits);assert.match(policeOrdersReport(c),/Recall squad/);assert.equal(serializeCity(c),saved);
