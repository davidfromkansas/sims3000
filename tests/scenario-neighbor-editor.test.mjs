...
change('customEvent0','resultText');assert.equal($('eventTextOutcomeLabel0').hidden,false);assert.equal($('eventMessage0').maxLength,1500);assert.equal($('eventX0').disabled,true);$('eventMessage0').value='Council decision: {population}';$('eventTextOutcome0').value='both';$('startCustom').onclick();assert.equal(city.scenario.definition.events[0].type,'resultText');assert.equal(city.scenario.definition.events[0].outcome,'both');assert.equal(city.scenario.definition.events[0].message,'Council decision: {population}');change('customEvent0','dialogText');assert.equal($('eventTextOutcomeLabel0').hidden,true);$('eventMessage0').value='New instructions';$('startCustom').onclick();assert.equal(city.scenario.definition.events[0].type,'dialogText');
