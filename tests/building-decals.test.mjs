...
const city=createCity();city.buildingDesigns[2]=d;const save=JSON.parse(serializeCity(city));assert.equal(save.version,153);assert.deepEqual(validateSave(save).buildingDesigns[2],d);save.version=151;assert.throws(()=>validateSave(save),/152/);delete save.buildingDesigns[2].decals;assert.equal(validateSave(save).version,153);
