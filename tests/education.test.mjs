...
const c=createCity();assert.ok(build(c,'library',[{x:21,y:20}]).ok);assert.equal(c.stats.schoolCoverage,0);assert.ok(c.stats.adultEducationCoverage>0);
...
unsupported.stats.adultEducationCoverage=0;for(let i=0;i<120;i++){month(maintained);month(unsupported);}assert.ok(maintained.civic.adultEducation>unsupported.civic.adultEducation);assert.ok(Math.abs(maintained.civic.youthEducation-30)<1e-9,'library does not teach youth');assert.ok(maintained.civic.adultEducation<80,'less educated graduates can still reduce adult average');
assert.ok(build(c,'school',[{x:21,y:22}]).ok);assert.ok(c.stats.schoolCoverage>0);const before=c.civic.youthEducation;for(let i=0;i<36;i++)month(c);assert.ok(c.civic.youthEducation>before);assert.ok(Math.abs(c.civic.education-(c.civic.youthEducation*educationWeights(c).youth+c.civic.adultEducation*educationWeights(c).adult))<1e-9);
