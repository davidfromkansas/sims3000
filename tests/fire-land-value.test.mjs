...
assert.equal(near.fireLandBonus,0);assert.ok(build(c,'fire',[{x:23,y:25}]).ok);assert.equal(near.fireLandBonus,far.fireLandBonus,'fire protection is uniform across the station area');assert.ok(far.fireLandBonus>0);assert.ok(near.landValue>before);assert.ok(c.stats.revenues.residential>revenue,'safer property feeds the existing tax base');
