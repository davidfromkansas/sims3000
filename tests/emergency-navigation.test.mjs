...
let locations=emergencyLocations(c);assert.equal(locations.length,3);assert.deepEqual(locations.map(t=>[t.x,t.y,t.tiles]),[[5,5,2],[80,80,2],[95,0,1]],'diagonal fires form one area and areas appear in ignition order even when their fire ages differ');
const navigate=createEmergencyNavigator();assert.equal(navigate(c).x,5);assert.equal(navigate(c).x,80);assert.equal(navigate(c).x,95);assert.equal(navigate(c).x,5);
assert.equal(navigate(c,'fire:485').x,5);assert.equal(navigate(c,'fire:999999'),null);assert.equal(navigate(c).x,80);
...
console.log('PASS: separate emergency areas, diagonal grouping and ignition order, cycling/direct choice, resolved/moving hazards, city and incident resets, saved fires and centered views across rotations/zoom without gameplay mutation.');
