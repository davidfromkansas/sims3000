...
const app=readFileSync(new URL('../dist/app.js',import.meta.url),'utf8'),source=app.slice(app.indexOf('function showQuery('),app.indexOf('function showIndustry(')),query=new Function('ui','with(ui){'+source+';return showQuery;}')(ui);query({x:12,y:19});assert.equal(ui.title,'Train station');assert.match(ui.html,/Stations on these track networks/);buttons['#stationRailMap'].onclick();assert.equal(ui.layer,'rail');buttons['#stationSubwayMap'].onclick();assert.equal(ui.layer,'subway');assert.equal(ui.closed,true);buttons['#stationTransitBudget'].onclick();assert.equal(ui.funding,true);
