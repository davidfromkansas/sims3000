...
const city=createCity('Skyline collection',false);for(const t of city.tiles)Object.assign(t,{terrain:'land',nature:false,elevation:0});const funds=city.funds;
for(const [i,type] of Object.keys(SKYLINE_LANDMARK_GEOMETRY).entries()){
 ...
 const old=JSON.parse(serializeCity(city));old.version=148;assert.throws(()=>validateSave(old),/version 149/);
...
assert.equal(city.funds,funds-2);assert.equal(landmarkRoots(city).length,2);
...
const prior=JSON.parse(serializeCity(createCity()));prior.version=148;assert.equal(validateSave(prior).version,VERSION);
console.log('PASS: two original skyline landmarks, four-view raster bounds and depth, unique free placement, far-corner demolition/rebuilding, scenario metrics and saved monthly continuation.');
