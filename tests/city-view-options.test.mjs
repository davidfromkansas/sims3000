...
ctx.drawImage=(...args)=>record('image',...args);const pumpDraw=()=>draws.image?.some(a=>a[1]===point.x-scene.unit*2.1/2&&a[2]===point.y+scene.unit/2-320*scene.unit*2.1/512);
scene.draw();assert.ok(draws.road.length);assert.ok(draws.track.length);assert.ok(draws.highway.length);assert.ok(pumpDraw());assert.ok(draws.sprite.some(a=>a[0]>=42&&a[0]<=44));
for(const key of Object.keys(draws))delete draws[key];scene.cityView=off;scene.draw();assert.equal(draws.road,undefined);assert.equal(draws.track,undefined);assert.equal(draws.highway,undefined);assert.equal(draws.line,undefined,'surface power-line strokes are hidden');assert.ok(!pumpDraw());assert.ok(!draws.sprite.some(a=>a[0]===0||a[0]>=42&&a[0]<=44));assert.ok(draws.sprite.some(a=>a[0]===27),'fire stays visible');assert.ok(draws.sprite.some(a=>a[0]===28),'rubble stays visible');assert.ok(draws.diamond.every(a=>a[4]===null),'grid and empty-zone outlines are hidden');
...
for(const key of Object.keys(draws))delete draws[key];scene.layer='power';scene.draw();assert.ok(draws.road.length);assert.ok(pumpDraw(),'diagnostic views retain their content');
