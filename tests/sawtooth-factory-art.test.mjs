...
const large=createCity('Factory lots',false,96),root=large.tiles[20*96+20];
for(let y=20;y<23;y++)for(let x=20;x<22;x++)Object.assign(large.tiles[y*96+x],{terrain:'land',elevation:0,type:'industrial',industry:'dirty',density:3,level:0,powered:true,watered:true,access:true});
const lot=formBuildingLot(large,root,2,3);assert.ok(lot);const lotVariants=new Set();
for(let seed=0;seed<32;seed++){large.seed=seed;const selected=cityZonedSprite(root,large,view);lotVariants.add(selected);for(const id of lot.ids)assert.equal(cityZonedSprite(large.tiles[id],large,view),selected);}
assert.deepEqual(lotVariants,new Set([6,79]));replaceBuildingStyle(large,'6@2x3',7);assert.equal(cityZonedSprite(root,large,view),7);
