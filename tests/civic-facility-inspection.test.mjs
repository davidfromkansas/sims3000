...
const hospital=at(15,19);let d=civicFacilityDetails(c,hospital);assert.equal(d.operating,true);assert.equal(d.residents,384);assert.equal(d.homes,6);assert.equal(d.capacity,1000);assert.equal(d.contribution,100);assert.equal(d.combinedCoverage,100);
c.civic.funding.health=1;recompute(c);d=civicFacilityDetails(c,hospital);assert.equal(d.capacity,10);assert.ok(Math.abs(d.contribution-d.combinedCoverage)<1e-9,'reported standalone contribution matches actual health coverage');assert.match(civicFacilityReport(c,hospital),/unmet demand/);
...
c.civic.underfunded.health=6;recompute(c);d=civicFacilityDetails(c,hospital);assert.equal(d.capacity,0);assert.equal(d.fundedCapacity,10);assert.ok(d.reasons.includes('Department workers are on strike'));assert.match(civicFacilityReport(c,hospital),/Not operating/);
