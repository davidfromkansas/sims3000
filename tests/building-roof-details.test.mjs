...
const city=createCity();city.buildingDesigns[2]=decorated;const saved=JSON.parse(serializeCity(city));assert.equal(saved.version,161);assert.deepEqual(validateSave(saved).buildingDesigns[2],decorated);saved.version=155;assert.throws(()=>validateSave(saved),/156/);delete saved.buildingDesigns[2].roofDetails;assert.equal(validateSave(saved).version,161);
