...
change('eventCondition0_1','funds');$('eventThreshold0_1').value=0;change('eventCondition0_2','population');$('eventThreshold0_2').value=1;$('eventLogic0').value='1 AND (2 OR 3)';$('eventLogic0').oninput();assert.equal($('eventMatch0').disabled,true);
$('rankName0').value='Nested success';change('eventConditionrank0','goalStatus3');$('eventThresholdrank0').value=2;change('eventConditionrank0_1','funds');$('eventThresholdrank0_1').value=0;change('eventConditionrank0_2','population');$('eventThresholdrank0_2').value=1;$('eventLogicrank0').value='1 AND (2 OR 3)';
...
tick(city);assert.equal(city.scenario.status,'playing');tick(city);assert.equal(city.scenario.status,'won');assert.equal(city.scenario.rank,'Nested success');assert.deepEqual(city.scenario.goalActivatedMonths,[2]);assert.deepEqual(city.scenario.goalMarks,[{satisfied:true,month:1}]);assert.deepEqual(validateSave(JSON.parse(serializeCity(city))).scenario,city.scenario);
const before=serializeCity(city);$('eventLogic0').value='1 AND (2 OR 4)';$('startCustom').onclick();assert.match($('customError').textContent,/empty condition row 4/);assert.equal(serializeCity(city),before);$('eventLogic0').value='1 AND (2 OR 3)';$('eventGoal0').value='1';$('startCustom').onclick();assert.match($('customError').textContent,/goal/i);assert.equal(saved,1);assert.equal(serializeCity(city),before,'invalid empty-row target does not overwrite the current scenario');
