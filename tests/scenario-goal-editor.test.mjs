...
change('customEvent0','addGoal');$('eventMonth0').value=2;$('eventGoal0').value='2';
...
$('startCustom').onclick();assert.equal($('customError').textContent,'');assert.equal(saved,1);assert.equal(city.scenario.definition.objectives.length,1);assert.equal(city.scenario.definition.events[0].goal,0,'sparse editor row maps to the correct compact objective index');assert.deepEqual(city.scenario.goalActivatedMonths,[null]);
tick(city);assert.equal(city.scenario.status,'playing');tick(city);assert.equal(city.scenario.status,'won');assert.deepEqual(city.scenario.goalActivatedMonths,[2]);assert.deepEqual(validateSave(JSON.parse(serializeCity(city))).scenario,city.scenario);
