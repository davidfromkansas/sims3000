...
for(let seed=0;seed<24;seed++){
 ...
 const old=structuredClone(city),current=structuredClone(city);const {commercialEmployed,industrialEmployed,civicEmployed,civicJobs,...legacy}=recomputeTransport(current);assert.deepEqual([civicJobs,civicEmployed],[0,0]);assert.deepEqual(legacy,reference(old),'stats seed '+seed);assert.ok(Math.abs(commercialEmployed+industrialEmployed-(legacy.commuters-legacy.unemployed))<1e-6);for(let i=0;i<city.tiles.length;i++)assert.deepEqual(current.tiles[i],old.tiles[i],'tile '+i+' seed '+seed);
}
