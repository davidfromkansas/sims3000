...
assert.equal(serviceRadius(c,at(20,23)),9);assert.equal(serviceRadius(c,at(30,23)),9);
...
for(const rotation of [0,1,2,3])for(const scale of [.75,2,4]){
 ...
 const [x,y]=renderer.transform(31,24);assert.deepEqual(arcs[0].slice(0,3),[(x+.5)*scale,(y+.5)*scale,9*scale]);assert.equal(arcs.length,2);assert.deepEqual(colors,['#f6ffe1']);
}
