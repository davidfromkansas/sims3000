...
const c=createCity('World landmarks',false,96);for(const t of c.tiles)Object.assign(t,{terrain:'land',nature:false,elevation:0});const funds=c.funds;
for(const [i,type]of [...MODELED_LANDMARKS].entries()){
 const x=3+(i%5)*9,y=10+Math.floor(i/5)*10;assert.ok(build(c,type,[{x,y}]).ok);assert.equal(c.tiles.filter(t=>t.type===type).length,LANDMARKS[type].size**2);assert.equal(planBuild(c,type,[{x,y:30}]).ok,false);assert.equal(CUSTOM_METRICS[type].read(c),1);
...
const forged=JSON.parse(serializeCity(c));for(let y=30;y<33;y++)for(let x=30;x<33;x++)Object.assign(forged.tiles[y*96+x],{type:'statueLiberty',root:30*96+30});assert.throws(()=>validateSave(forged),/one of each landmark/);
