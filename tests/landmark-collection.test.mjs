...
for(const [i,type]of [...MODELED_LANDMARKS].entries()){
 const x=10+i*12;assert.ok(build(c,type,[{x,y:10}]).ok);assert.equal(c.tiles.filter(t=>t.type===type).length,9);assert.equal(planBuild(c,type,[{x,y:30}]).ok,false);assert.equal(CUSTOM_METRICS[type].read(c),1);
