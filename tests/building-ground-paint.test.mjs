...
const city=createCity();city.buildingDesigns[2]=grass;const saved=JSON.parse(serializeCity(city));assert.equal(saved.version,151);assert.deepEqual(validateSave(saved).buildingDesigns[2],grass);saved.version=150;assert.throws(()=>validateSave(saved),/151/);delete saved.buildingDesigns[2].groundPaint;assert.equal(validateSave(saved).version,151);
