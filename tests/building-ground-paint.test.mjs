...
assert.equal(paintGround(grass,[0],-1)[0],'0');for(const bad of ['',null,'!'.repeat(100),'0'.repeat(99)])assert.throws(()=>validateGroundPaint(bad));assert.throws(()=>paintGround(base,[-1],5));assert.throws(()=>paintGround(base,[0],7));
...
const city=createCity();city.buildingDesigns[2]=grass;const saved=JSON.parse(serializeCity(city));assert.equal(saved.version,155);assert.deepEqual(validateSave(saved).buildingDesigns[2],grass);saved.version=150;assert.throws(()=>validateSave(saved),/151/);delete saved.buildingDesigns[2].groundPaint;assert.equal(validateSave(saved).version,155);
