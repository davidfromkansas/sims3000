...
assert.equal(created,12);assert.equal(put,12);assert.equal(drawn,24);assert.equal(serializeCity(city),saved);const loaded=validateSave(JSON.parse(saved));tick(city);tick(loaded);assert.deepEqual(city.stats,loaded.stats);
...
const actual=new Function('STRUCTURES','MODELED_POWER','MODELED_REWARDS','drawCityReward','return function(city,t,c,p,u,fade){'+body+'}')(STRUCTURES,new Set(),MODELED_REWARDS,(...args)=>calls.push(args));
const scene=Object.assign(Object.create(CityRenderer.prototype),{getCity:()=>city,rotation:0,zoom:1,w:1000,h:700,pan:{x:0,y:0}});
for(let rotation=0;rotation<4;rotation++){scene.rotation=rotation;calls.length=0;for(const t of city.tiles)if(MODELED_REWARDS.has(t.type))actual.call(scene,city,t,ctx,scene.project(t.x,t.y),scene.unit,.4);assert.equal(calls.length,3);for(const call of calls){const type=call[1],root=city.tiles.find((t,i)=>t.type===type&&t.root===i),size=REWARDS[type].size,center=scene.project(root.x+(size-1)/2,root.y+(size-1)/2);assert.deepEqual(call,[ctx,type,rotation,center.x,center.y+scene.unit/2,scene.unit*size*2.1,.4]);}}
console.log('PASS: three reward buildings in twelve distinct bounded views, cached rasters, actual earned placement/save continuity, and once-per-footprint rendering aligned through four rotations.');
