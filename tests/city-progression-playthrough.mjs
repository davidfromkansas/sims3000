...
assert.equal(c.stats.population,32016);assert.equal(c.funds,866604);assert.equal(c.rewards.earned.mayorHouse,null);assert.equal(c.rewards.earned.cityHall,null);assert.equal(c.finance.nextLoanId,1);assert.equal(c.stats.uncollectedWaste,0);const u=utilityTrendSnapshot(c);assert.ok(u.powerUnserved<1e-7);assert.ok(u.waterUnserved<1e-7);assert.equal(serializeCity(c),serializeCity(checkpoint));console.log('PASS: ordinary empty-map construction reaches 32,016 residents, recovers from landfill exhaustion, requires stronger approval for civic rewards without grants or loans, and preserves the final year through save/load.');
