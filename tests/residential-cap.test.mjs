...
const cultural=createCity('Museum capacity'),base=cultural.stats.residentialCap.limit;assert.ok(build(cultural,'museum',[{x:23,y:25}]).ok);const museum=cultural.tiles[25*48+23];assert.equal(museum.serviceActive,true);assert.equal(cultural.stats.residentialCap.limit,base+9000);assert.match(residentialCapReport(cultural),/Museum × 1: \+9,000/);assert.equal(validateSave(JSON.parse(serializeCity(cultural))).stats.residentialCap.limit,base+9000);
const population=cultural.stats.population;cultural.civic.funding.education=0;recompute(cultural);assert.equal(cultural.stats.residentialCap.limit,base);assert.equal(cultural.stats.population,population,'losing capacity does not evict residents');cultural.civic.funding.education=100;recompute(cultural);assert.equal(cultural.stats.residentialCap.limit,base+9000);museum.fire=10;recompute(cultural);assert.equal(cultural.stats.residentialCap.limit,base);museum.fire=0;recompute(cultural);assert.equal(cultural.stats.residentialCap.limit,base+9000);assert.ok(build(cultural,'museum',[{x:27,y:21}]).ok);assert.equal(cultural.stats.residentialCap.limit,base+18000,'operating museums add capacity independently');
