...
const filters=new Map(['landmarkSearch','landmarkAvailability','clearLandmarkFilters','landmarkCount','landmarkEmpty'].map(id=>[id,{value:id==='landmarkAvailability'?'all':'',focus(){this.focused=true;}}])),cards=[...MODELED_LANDMARKS].map(key=>({dataset:{landmarkCard:key},hidden:false}));
...
globalThis.document={createElement(){created++;return {width:0,height:0,getContext:offscreen};},getElementById:key=>canvases.get(key)??labels.get(key)??filters.get(key),querySelectorAll:s=>s==='[data-landmark-turn]'?turns:s==='[data-landmark-card]'?cards:places};
...
assert.equal(created,MODELED_LANDMARKS.size);
...
assert.equal(created,MODELED_LANDMARKS.size*4,'one shared raster per model/view, reused on wrapping');assert.equal(serializeCity(city),before);
...
const search=filters.get('landmarkSearch'),availability=filters.get('landmarkAvailability'),clear=filters.get('clearLandmarkFilters'),visible=()=>cards.filter(c=>!c.hidden).map(c=>c.dataset.landmarkCard);
turns.find(t=>t.dataset.landmarkTurn==='bigBen'&&t.dataset.step==='1').onclick();
search.value='tOrOnTo';search.oninput();assert.deepEqual(visible(),['cnTower']);
search.value='empire york';search.oninput();assert.deepEqual(visible(),['empireStateBuilding'],'all search words match across the name and location');
search.value='HELSINKÍ';search.oninput();assert.deepEqual(visible(),['helsinkiCathedral'],'search ignores letter accents');
availability.value='available';availability.onchange();assert.deepEqual(visible(),[]);assert.equal(filters.get('landmarkEmpty').hidden,false);
availability.value='placed';availability.onchange();assert.deepEqual(visible(),['helsinkiCathedral']);
search.value='<script>';search.oninput();assert.deepEqual(visible(),[]);assert.match(filters.get('landmarkCount').textContent,/^0 of/);
clear.onclick();assert.equal(visible().length,MODELED_LANDMARKS.size);assert.equal(clear.disabled,true);assert.equal(search.focused,true);assert.equal(labels.get('landmarkDirection-bigBen').textContent,'East view','filtering retains independently rotated previews');assert.equal(serializeCity(city),before);
console.log('PASS: name/location and multiword search, accent folding, placed/available filters, empty result feedback, clear/focus restoration and unchanged city.');
