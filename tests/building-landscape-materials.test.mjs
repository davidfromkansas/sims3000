...
for(const material of [5,6]){
 ...
 const city=createCity();city.buildingDesigns[2]=design;const saved=JSON.parse(serializeCity(city));assert.equal(saved.version,161);assert.deepEqual(validateSave(saved).buildingDesigns[2],design);
...
const old=JSON.parse(serializeCity(createCity()));old.version=149;assert.equal(validateSave(old).version,161);
