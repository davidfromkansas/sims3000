...
const app=readFileSync(new URL('../dist/app.js',import.meta.url),'utf8'),querySource=app.slice(app.indexOf('function showTileQuery('),app.indexOf('function showIndustry(')),controls={};const ui={canReplaceBuilding,city:c,LABEL,ZONES,idx:(x,y)=>y*96+x,buildingLotReport,canPreserve:()=>false,$:id=>controls[id],dialog(name,html){ui.name=name;ui.html=html;}};const query=new Function('ui','with(ui){'+querySource+';return showTileQuery;}')(ui);for(const id of lot.ids){query(c.tiles[id]);assert.equal(ui.name,'Residential');assert.ok(ui.html.startsWith(rootReport));assert.match(ui.html,/Replace building style/);}
