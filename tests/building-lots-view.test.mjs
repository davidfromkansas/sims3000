...
import {baseZonedSprite,zonedSprite,canReplaceBuilding} from '../dist/building-art.js';
...
const actual=new Function('buildingLotPlacement','zonedSprite','designForTile','defaultLotDesign','drawDesignedBuilding','return function(city,t,c,u,fade){'+body+'}')(buildingLotPlacement,zonedSprite,designForTile,defaultLotDesign,(...args)=>calls.push(args));r.sprite=(...args)=>calls.push(args);r.layer='city';const ctx={fillText:(...args)=>markers.push(args)};
...
const app=readFileSync(new URL('../dist/app.js',import.meta.url),'utf8'),querySource=app.slice(app.indexOf('function showQuery('),app.indexOf('function showIndustry(')),controls={};const ui={canReplaceBuilding,city:c,LABEL,ZONES,idx:(x,y)=>y*96+x,buildingLotReport,canPreserve:()=>false,$:id=>controls[id],dialog(name,html){ui.name=name;ui.html=html;}};const query=new Function('ui','with(ui){'+querySource+';return showQuery;}')(ui);for(const id of lot.ids){query(c.tiles[id]);assert.equal(ui.name,'Residential');assert.ok(ui.html.startsWith(rootReport));assert.match(ui.html,/Replace building style/);}
ui.canPreserve=canPreserve;ui.designateHistorical=designateHistorical;ui.recompute=recompute;ui.undo=[1];ui.AUTO='auto';let saves=0;ui.persist=()=>saves++;ui.update=()=>{};ui.notify=message=>assert.fail(message);controls['#makeHistorical']={checked:true};query(c.tiles[lot.ids.at(-1)]);controls['#makeHistorical'].onchange();assert.ok(lot.ids.every(i=>c.tiles[i].historicalLevel===1));assert.equal(saves,1);assert.deepEqual(ui.undo,[]);
console.log('PASS: one draw per rectangular footprint in all rotations, large-map placement and scale, shared member queries, aggregate population/services, root-only scenario counts, abandonment counts and unchanged query state.');
