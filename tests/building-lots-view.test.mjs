...
const actual=new Function('buildingLotPlacement','zonedSprite','designForTile','defaultLotDesign','drawDesignedBuilding','drawRecentConstruction','return function(city,t,c,u,fade){'+body+'}')(buildingLotPlacement,zonedSprite,designForTile,defaultLotDesign,(...args)=>calls.push(args),drawRecentConstruction);r.sprite=(...args)=>calls.push(args);r.layer='city';r.ctx={save(){},restore(){},beginPath(){},moveTo(){},lineTo(){},closePath(){},fill(){},stroke(){}};const ctx={fillText:(...args)=>markers.push(args)};
...
const app=readFileSync(new URL('../dist/app.js',import.meta.url),'utf8'),querySource=app.slice(app.indexOf('function showTileQuery('),app.indexOf('function showIndustry(')),controls={};const ui={recentConstructionNote,canReplaceBuilding,city:c,LABEL,ZONES,idx:(x,y)=>y*96+x,buildingLotReport,canPreserve:()=>false,$:id=>controls[id],dialog(name,html){ui.name=name;ui.html=html;}};const query=new Function('ui','with(ui){'+querySource+';return showTileQuery;}')(ui);for(const id of lot.ids){query(c.tiles[id]);assert.equal(ui.name,'Residential');assert.ok(ui.html.startsWith(rootReport));assert.match(ui.html,/Replace building style/);}
