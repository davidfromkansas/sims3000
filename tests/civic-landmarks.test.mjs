...
for(const [key,pop,cap,jobs,cost] of [['historicStatue',35000,10000,1,0],['lighthouse',15000,6000,8,5000]]){
 ...
 const saved=JSON.parse(serializeCity(c));assert.equal(saved.version,160);assert.equal(validateSave(saved).stats.residentialCap.limit,25000+cap);const old=structuredClone(saved);old.version=138;assert.throws(()=>validateSave(old));
