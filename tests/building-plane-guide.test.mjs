...
for(const plane of ['horizontal','xz','yz']){const p=BUILDING_EDIT_PLANES[plane];for(const reference of [0,9,p.rows*10-1])assert.deepEqual(buildingPlaneGuide({plane,slice:0,reference}),buildingPlaneGuide({plane,slice:0,pending:{cells:[reference]}}));for(const reference of [-1,p.rows*10,1.5])assert.equal(buildingPlaneGuide({plane,slice:0,reference}).length,1);}
