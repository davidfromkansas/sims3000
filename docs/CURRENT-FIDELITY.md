...
The architecture and large-city responsiveness milestone is merged through PR #192 (main 39ea6621, schema 157). The named scenario-variable milestone is merged in PR #194 at schema 158. Explicit ending ranks are merged in PR #195 at schema 159. Live-value comparisons use schema 160, passed all 337 regression commands, and have a final browser-verified visibility correction before their PR. The current catalog has 22 modeled landmarks, 14 rewards, 12 ordinances and 12 prepared challenges. Publication after live version 150 remains pending explicit Sites source-export approval. This is a playable reconstruction with substantial incomplete scope, not verified full SimCity 3000 Unlimited parity. A passing suite count does not establish complete manual fidelity.

## Historical architecture working notes

The entries in this section retain their release-time state, including old counts, pending PRs and temporary Mac-lock limitations. They are not the current project status. Current scope is summarized in the table below and the linked milestone records.

Current architecture work adds a city-wide replacement manager with an all-footprint view, zone/footprint filters, future-building selection, whole-set reset, independent preview rotation, hold-to-compare, direct custom-file drafts and a reusable browser-local custom-building list. The library has 19 supplied RCI styles and 14 natural source styles. Limestone office block, Sawtooth factory and Courtyard apartments each have four native Blender views for all 25 supported footprints (300 rendered views across three designs). Saved custom models take priority. Browser checks cover selection, reset, saved restoration, all camera directions and ordinary starter-town continuation. Five-tile customization has focused editor/save checks, and all expanded supplied images passed staging validation. The office/factory expanded browser fixture has been reviewed from North, East and South; its West review remains pending because the Mac is locked. The new courtyard artwork still needs in-game browser review. Reward/opportunity replacement, full block shapes, broader catalog and performance/player acceptance remain outstanding. See [the architecture work record](ARCHITECTURE-COLLECTION-MILESTONE.md) for scoped evidence.

The integrated architecture-collection-22 regression run completed with 306 PASS records and exit code 0, including all eleven landmark models, the four-memorial batch, gallery rotation, courtyard artwork, the transactional building manager and newer editor controls. Browser acceptance of the newest manager/list flows remains pending; the Mac is locked.

All eleven current landmarks now have directional models and fitted, independently rotatable gallery previews. The Eiffel Tower and Great Pyramid retain their existing footprints and saved types. Four new American memorial landmarks are integrated at schema 148; their footprints are authored reconstructions. The full cache architecture-collection-22 regression passes, including actual renderer placement through all map directions; browser visual review remains pending.

Current catalog audit: runtime code contains thirteen landmarks and fourteen rewards. EA’s product listing describes more than 95 landmarks; the development inventory now records 74 standard candidates, 25 Unlimited additions and the existing downloadable extension separately. Thirteen entries map to runtime types and 87 do not. CN Tower remains a secondary-source discrepancy, and architectural identity/composition checks remain unfinished. See [landmark catalog coverage](LANDMARK-CATALOG-AUDIT.md).

Architect follow-up at cache architecture-collection-23 adds pre-click target highlights in the edit grid and 3D model, including keyboard focus. Three focused suites pass; these changes are not yet in PR #192 and browser acceptance remains pending.

Architect props at cache architecture-collection-25 add tree/car placement on visible ground or roofs, independent direction, target-size preview, removal and shared construction undo/redo. Props survive support removal, including a prop-only model, and keep their latest positions/removals across height-layout and independent-layer modes. Portable format 10 and city schema 149 preserve them. All 308 registered regression suites passed at cache architecture-collection-25 (309 PASS records, exit 0), including editor, model, mode-switch, actual designer-save and four-view zoom/pan placement checks. Adjacent-wall cropping, the wider eight-category prop catalog and browser visual acceptance remain unfinished. These follow-up changes are uncommitted and outside draft PR #192.

The unpublished prop follow-up at cache architecture-collection-26 now has twelve original models and a category selector covering all eight manual categories. Forty-eight enlarged directional views were reviewed. A thin-detail raster fix preserves visibility on narrow five-tile lots. Seven focused suites pass, including new catalog, save/continuation and actual category-control checks; the last full regression remains cache 25 (308 suites). Complete original prop inventory, physical clipping against adjacent walls and browser visual acceptance remain outstanding.

At cache architecture-collection-27, prop surfaces are geometrically cropped against occupied columns, cubes and all four wedge orientations. Tests cover known intersection areas, narrow lots, overhangs, layer gaps and roof contact; sixteen enlarged construction/prop views were inspected. The full 310-command regression passed with 312 PASS records and exit code 0 (`/tmp/sims3000-prop-clipping-tests.log`). This implements a documented behavior using an original clipping algorithm; original algorithm parity, actual browser appearance and editor-render performance remain unverified.

At cache architecture-collection-28, unchanged prop previews reuse up to four directional frames per drawing context, with content-based invalidation for construction, wedge, footprint and prop edits. Four focused cache/designer/clipping/editor suites pass; the last full regression remains the 310-suite cache-27 run. A mocked-context stress check reduced reused prop-frame preparation to approximately 0.008–0.009 ms, excluding browser compositing and the rest of the editor. Actual browser responsiveness remains unverified.

At cache architecture-collection-29, the selected prop has a fitted palette preview using the actual geometry and lighting, following both its direction and the building viewing angle. Captions/accessible labels distinguish these directions, and unchanged refreshes reuse the image. Four focused collection/editor/designer/cache suites pass, including all 192 model/direction/view combinations. A fresh browser availability check still found the Mac locked; visual acceptance remains pending.

At cache architecture-collection-30, Erase props removes the frontmost visible prop directly from the model preview through the construction undo path. Depth-aware picking respects wall occlusion, clipping, overlapping props, four views, rectangular lots and zoom/pan. Four focused picking/editor/designer/paint suites pass. The last full regression remains the 310-suite cache-27 run; browser acceptance is pending.

At cache architecture-collection-31, Empire State Building and CN Tower add two original modeled landmarks with free unique placement, scenario counts and saved continuation. Eight enlarged directional views and focused gameplay/gallery/renderer checks pass. Footprints are authored 3×3 approximations; CN Tower retains its directory-source discrepancy flag. The full 313-command regression passed with 318 PASS records and exit code 0 in `/tmp/sims3000-skyline-batch-tests.log`. Browser review remains pending.

At cache architecture-collection-32, landmark browsing adds name/location search and placed/available filters, live result counts and a clearable empty state. The actual gallery-control suite passes and verifies preserved preview rotations and unchanged city state. These are adapted browser browsing controls, not a claim of original UI reproduction. Last full regression: 313 suites at cache 31. Browser acceptance remains pending.

At cache architecture-collection-33, selecting an existing prop copies its category/type/direction without editing, and drag erasing removes multiple visible props in one undoable stroke. Four focused prop-editor/construction/paint/designer suites pass. Blue selection/erase outlines are implemented; the manual’s full blue-tint highlight remains incomplete. Last full regression: 313 suites at cache 31; browser acceptance is pending.

At cache architecture-collection-34, existing-prop selection and erase previews tint visible surfaces blue with retained shading, including all props marked in a drag. Depth-tested overlays respect walls and overlapping props. Separate overlay caches preserve the normal artwork cache. Six focused suites pass; this supersedes the earlier outline-only highlighting gap. Last full regression remains 313 suites at cache 31; exact original tint colors and browser visual acceptance are unverified.

At cache architecture-collection-35, a cached whole-model overview shows the current main-preview area and supports click/keyboard recentering while retaining zoom. Three focused overview/camera/designer suites pass. The manual’s special Construct-mode hexagonal frame behavior and browser layout acceptance remain unfinished. Last full regression remains 313 suites at cache 31.

At cache architecture-collection-36, checkpoint 7 provides seven architecture feedback exercises and direct designer/library/landmark launch controls, preserving existing player notes and the original three checkmarks. Actual feedback persistence/export and app launch routing pass. The full 315-command regression passed with 324 PASS records and exit code 0 in `/tmp/sims3000-architecture-feedback-tests.log`. Browser acceptance remains blocked by the locked Mac, and publication requires the separately pending Sites source-export authorization.
...
| Rewards, landmarks and architecture | `rewards.js`, `landmarks.js`, `building-art.js`: 14 reward types, 22 modeled landmarks, and original directional building assets | 78 of the 100 inventoried landmarks remain unmapped; complete catalog/style coverage and broader visual acceptance remain open |
...
| Building replacement, pp.72–84 | `building-replacement-ui.js`, `building-art.js`, `building-designs.js`, `building-designer-ui.js`: filterable library, citywide substitution, tower/block/layer models, surface and ground paint, anchored details, props, portable models, revert and saved overrides | Full original block/detail/texture/prop catalogs and original-format interoperability remain unverified; broader editing acceptance remains open. The prior four-style/tower-only limitation is obsolete |
...
| Scenarios and creator, pp.24–26, 155–197 | `scenarios.js`, `custom-scenarios.js`, `scenario-events.js`, `scenario-text.js`: 12 prepared challenges, custom goals/streaks, repeated/conditional events, variables/calculations, structured If/Else blocks and subroutines, dynamic goals, outcome messages and portable challenge cities | Exact shipped-content coverage and command-by-command acceptance remain open. The current source supports one to 32 named counters, reference-preserving collection edits and saved continuation; merged in PR #194. Scripted endings now support an explicit rank selection in PR #195. Four goals/events and bounded routines still constrain authoring |
| Presentation and sound | `renderer.js`, `game-audio.js`: original directional sprites, procedural city/disaster motion, original procedural music, effects and volume controls | Full soundtrack/effects, broader animation, four-facing sprites and continued visual/performance refinement |

Catalog snapshot above was refreshed against runtime exports after main merge `39ea6621`: 14 rewards, 22 landmarks, 12 ordinances, 12 prepared challenges and 23 event types. Counts describe implemented entries, not a completeness percentage. Historical checkpoint notes below retain their release-time descriptions and must not override current source evidence.
...
1. Merge the validated live-comparison milestone, then audit remaining scenario commands individually rather than treating already-implemented blocks and goal activation as absent.
...
3. Expand the original building, landmark, reward, business and ordinance catalogs while extending the existing replacement library and block/layer editor.
...
Civic advisor checkpoint: original portraits for Maria Montoya and Randall Shoop accompany current safety, health, education and aura priorities, with map, planning and funding shortcuts. See [civic advisor briefings](CIVIC-ADVISOR-BRIEFINGS-MILESTONE.md). All 230 suites passed; final markup fix passed targeted validation. Browser layout checked for Maria; remaining live shortcut checks interrupted by a locked Mac. Source only, publication pending.
