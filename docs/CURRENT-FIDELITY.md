...
The architecture and large-city responsiveness milestone is merged through PR #192 (main 39ea6621, schema 157). The named scenario-variable milestone now uses schema 158 and passed all 335 regression commands and is ready for its PR. The current catalog has 22 modeled landmarks, 14 rewards, 12 ordinances and 12 prepared challenges. Publication after live version 150 remains pending explicit Sites source-export approval. This is a playable reconstruction with substantial incomplete scope, not verified full SimCity 3000 Unlimited parity. A passing suite count does not establish complete manual fidelity.
...
1. Merge the validated named-variable milestone, then audit remaining scenario commands individually rather than treating already-implemented blocks and goal activation as absent.
