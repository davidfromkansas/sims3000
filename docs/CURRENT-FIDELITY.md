...
The architecture and large-city responsiveness milestone is merged through PR #192 (main 39ea6621, schema 157). The named scenario-variable milestone is merged in PR #194 at schema 158. Explicit ending ranks are merged in PR #195 at schema 159. Live-value comparisons are merged through PR #196 at schema 160. The East Asian landmark milestone is in progress at schema 161. The current working catalog has 25 modeled landmarks, 14 rewards, 12 ordinances and 12 prepared challenges. Publication after live version 150 remains pending explicit Sites source-export approval. This is a playable reconstruction with substantial incomplete scope, not verified full SimCity 3000 Unlimited parity. A passing suite count does not establish complete manual fidelity.
...
| Rewards, landmarks and architecture | `rewards.js`, `landmarks.js`, `building-art.js`: 14 reward types, 25 modeled landmarks, and original directional building assets | 78 of the 100 inventoried landmarks remain unmapped; complete catalog/style coverage and broader visual acceptance remain open |
