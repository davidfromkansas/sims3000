...
Current catalog audit: runtime code contains eleven landmarks and fourteen rewards. EA’s product listing describes more than 95 landmarks; the development inventory now records 74 standard candidates, 25 Unlimited additions and the existing downloadable extension separately. Eleven entries map to runtime types and 89 do not. CN Tower remains a secondary-source discrepancy, and architectural identity/composition checks remain unfinished. See [landmark catalog coverage](LANDMARK-CATALOG-AUDIT.md).
