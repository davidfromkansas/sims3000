...
Variable maps: `createCity(name, starter, size)` and `generateCity({size})` accept 48, 96, 128 or 256. Persist `city.size`; derive per-city coordinate helpers with `cityGrid(city)`. The legacy exported `SIZE`, `idx`, `inside` and `selection` default to 48 for older callers; pass the size argument to the latter helpers when operating on another map. Tile `mapSize` and farm size are derived and never accepted from serialized tile fields. Scenario definitions carry a matching mapSize. Do not introduce a mutable global active-map size.
